...
import type { AuditLog, EnrollmentItem } from '@/types/admin';
import { createClient } from '@/lib/supabase/client';
import { useAlert, useConfirm } from '@/components/ui/alert-modal';
import { createUserInAuth, updateUserPasswordInAuth } from '@/app/(protected)/admin/actions';
...
export function useAdminDashboardData() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            setEnrollments(
                (enrollmentData || []).map((enr) => {
                    const u = Array.isArray(enr.users) ? enr.users[0] : enr.users;
                    return {
            ...
            type EventRow = { id: string; title: string; date: string; type: string; class_id?: string };
            setEventsList(
                (eventData || []).map((evt: EventRow) => ({
                    id: evt.id,
                    title: evt.title,
                    date: evt.date,
                    type: (evt.type as 'exam' | 'task' | 'academic') || 'academic',
            ...
            type AuditRow = { id: string; user_id?: string; user_name?: string; action: string; ip_address?: string; created_at: string };
            const userMap = new Map(
                fetchedUsers.map((u: User) => [u.id, u.name]),
            );
            setAuditLogs(
                (logData || []).map((log: AuditRow) => ({
                    id: log.id,
                    user_id: log.user_id || '',
                    user_name:
            ...
            console.error('Gagal mengambil data:', (err instanceof Error ? err.message : String(err)));
        } finally {
            setUsersLoading(false);
            setAuditLoading(false);
        }
    ...
    const fetchCoursesPage = useCallback(
        async (page: number, search: string) => {
            ...
            try {
                ...
                console.error('Gagal mengambil data kelas:', (err instanceof Error ? err.message : String(err)));
            } finally {
    ...
    const fetchStudents = useCallback(
        async (search: string) => {
            ...
            try {
                ...
                console.error('Gagal mengambil data mahasiswa:', (err instanceof Error ? err.message : String(err)));
            } finally {
    ...
    const handleAddUser = async (data: {
        ...
        try {
            ...
            showError('Gagal mendaftarkan user: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleUpdateUser = async (
        ...
        try {
            ...
            showError('Gagal memperbarui user: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleDeleteUser = async (id: string) => {
        ...
        try {
            ...
            showError('Gagal menghapus user: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleAddEvent = async (data: {
        ...
        try {
            ...
            showError('Gagal menambahkan agenda: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleDeleteEvent = async (id: string) => {
        ...
        try {
            ...
            showError('Gagal menghapus agenda: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleAddCourse = async (data: {
        ...
        try {
            ...
            showError('Gagal menambahkan kelas: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleDeleteCourse = async (id: string) => {
        ...
        try {
            ...
            showError('Gagal menghapus kelas: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleEnrollStudent = async (courseId: string, studentId: string) => {
        ...
        try {
            ...
            showError('Gagal mendaftarkan mahasiswa: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
    ...
    const handleUnenrollStudent = async (
        ...
        try {
            ...
            showError('Gagal mengeluarkan mahasiswa: ' + (err instanceof Error ? err.message : String(err)));
        }
    };
