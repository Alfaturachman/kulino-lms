...
export default function LecturerCourseDetailClient({
    ...
    const [activeTab, setActiveTab] = useState<
        'materi' | 'tugas' | 'penilaian' | 'mahasiswa'
    >('materi');
    ...
    const [attendanceForm, setAttendanceForm] = useState<{
        [studentId: string]: 'present' | 'absent' | 'sick' | 'permission';
    }>({});
    ...
    const [matType, setMatType] = useState<'video' | 'pdf' | 'link' | 'ppt'>(
        'pdf',
    );
        ...
        if (!isSupabaseConfigured) {
            ...
            setEnrolledStudents([
                ...
                },
                {
                    ...
                    name: 'Citra Kirana',
                    nim: '220102002',
                    email: 'citra.kirana@mhs.dinus.ac.id',
                    attendance: '92.0%',
                    status: 'Aktif',
                },
                {
                    ...
                    name: 'Bima Sakti',
                    nim: '220102005',
                    email: 'bima.sakti@mhs.dinus.ac.id',
                    attendance: '90.2%',
                    status: 'Aktif',
                },
                {
                    ...
                    name: 'Dewi Ayu',
                    nim: '220102078',
                    email: 'dewi.ayu@mhs.dinus.ac.id',
                    attendance: '71.0%',
                ...
                },
                {
                    ...
                    name: 'Galih Prasetya',
                    nim: '220102044',
                    email: 'galih.prasa@mhs.dinus.ac.id',
                    attendance: '68.0%',
                    status: 'Perlu Perhatian',
                },
            ]);
            ...
            if (enrollmentsData) {
                const studentList = enrollmentsData.map((e: any) => {
                    ...
                    if (studentAtts.length > 0) {
                        const presentCount = studentAtts.filter(
                            (a: any) =>
                                a.status === 'present' ||
                                a.status === 'permission',
                        ).length;
                        const pct = (presentCount / studentAtts.length) * 100;
                        attendancePct = `${pct.toFixed(1)}%`;
                        status = pct < 75 ? 'Perlu Perhatian' : 'Aktif';
                    }

                    return {
                        id: studentId,
                        name: u?.name || 'Mahasiswa Kulino',
                        nim: u?.nim_nip || '-',
                        email: u?.email || '-',
    ...
    const fetchWeekAttendance = useCallback(
        async (week: number) => {
            if (!isSupabaseConfigured) {
                const initialForm: any = {};
                enrolledStudents.forEach((s) => {
                    initialForm[s.id] = 'present';
                });
            ...
            try {
                ...
                data?.forEach((att: any) => {
                    initialForm[att.student_id] = att.status;
                });
                setAttendanceForm(initialForm);
            } catch (err) {
                console.error('Gagal mengambil presensi mingguan:', err);
            }
        },
        [isSupabaseConfigured, enrolledStudents, supabase, courseId],
    );

    useEffect(() => {
        if (showAttendanceModal) {
            fetchWeekAttendance(selectedWeek);
        }
    }, [showAttendanceModal, selectedWeek, fetchWeekAttendance]);

    const handleSaveAttendance = async () => {
        setSavingAttendance(true);
        try {
            if (!isSupabaseConfigured) {
                alert('Presensi berhasil disimpan! (Simulasi Mode Mock)');
                setShowAttendanceModal(false);
                return;
            }

            const upsertData = Object.entries(attendanceForm).map(
                ([studentId, status]) => ({
                    class_id: courseId,
            ...
            const { error } = await supabase
                .from('attendance')
                .upsert(upsertData, {
                    onConflict: 'class_id,student_id,week_no',
                });
    ...
    const handleAddAssignment = async (e: React.FormEvent) => {
        ...
        if (isSupabaseConfigured) {
            try {
                const allowedFormatsArray = asmFormats
                    .split(',')
        ...
        <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="space-y-2">
                        ...
                        <p className="text-[13px] text-muted max-w-2xl leading-relaxed">
                            Manajemen bahan ajar, penugasan, absensi dan
                            penilaian mahasiswa kelas {course.class_name}.
                        </p>
                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        value={asmWeight}
                                        onChange={(e) =>
                                            setAsmWeight(e.target.value)
                                        }
                                        required
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <Label htmlFor="formats-asm">
                                        Format File
                                    </Label>
                                    <Input
                                        id="formats-asm"
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-[12px] border-collapse">
                                    ...
                                    <tbody className="divide-y divide-border/60">
                                        {submissions.map((sub) => {
                                            const asmDetail = assignments.find(
                                                (a) =>
                                                    ...
                                                    <td className="p-3 text-right">
                                                        {isGraded ? (
                                                            <div className="flex items-center justify-end gap-2">
                                                                <span className="font-bold text-success text-[13px]">
                                                                    {sub.grade}
                                                                    /100
                                                                </span>
                                                                <button
                        ...
                        {gradingSubId ? (
                            <Card className="p-5 space-y-4 bg-iris-50/20 border-iris-100">
                                ...
                                <div className="text-[12px] space-y-1.5 text-muted bg-white p-3 rounded-lg border border-border/80">
                                    ...
                                    <p>
                                        Berkas:{' '}
                                        <span className="font-mono underline text-iris-600 cursor-pointer">
                                            {
                                                submissions.find(
                                                    (s) =>
                                                        s.id === gradingSubId,
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <Label htmlFor="grade-feedback-val">
                                            Feedback / Catatan Perbaikan
                                        </Label>
                                        <Textarea
                            ...
                            <Card className="p-6 text-center text-muted text-[12px] flex flex-col items-center justify-center py-12">
                                ...
                                />
                                <span>
                                    Pilih salah satu baris tugas mahasiswa di
                                    tabel untuk mulai menilai dan memberi umpan
                                    balik koreksi.
                                </span>
                            </Card>
                        )}
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <Badge variant="blue">
                                {enrolledStudents.length} Peserta Aktif
                            </Badge>
                        </div>
                        <Button
                            size="sm"
                        ...
                        >
                            <UserCheck size={14} /> Kelola Presensi
                        </Button>
                    </div>
                                ...
                                <tbody className="divide-y divide-border/60">
                                    {enrolledStudents.map((std, idx) => (
                                        ...
                                        >
                                            ...
                                            <td
                                                className={`p-3 text-center font-bold ${parseFloat(std.attendance) < 75 ? 'text-danger' : 'text-ink'}`}
                                            >
                                                {std.attendance}
                                            </td>
                                            <td className="p-3 text-right">
            ...
            {showAttendanceModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 animate-fade-in">
                    <Card className="w-full max-w-md bg-surface p-6 shadow-2xl space-y-4 border border-border">
                        <div className="flex items-center justify-between border-b border-border/80 pb-3">
                            <h3 className="text-sm font-bold text-ink flex items-center gap-2">
                                ...
                                />{' '}
                                Kelola Presensi Kuliah
                            </h3>
                            <button
                                onClick={() => setShowAttendanceModal(false)}
                                className="text-[12px] text-muted hover:text-ink font-semibold cursor-pointer"
                            >
                                Tutup
                            </button>
                        </div>

                        <div className="flex items-center gap-3 bg-surface2/60 p-3 rounded-lg border border-border">
                            <Label
                                htmlFor="week-select"
                                className="text-[12px] font-semibold text-ink2"
                            >
                                Pilih Pertemuan:
                            </Label>
                            <select
                                ...
                                className="bg-white border border-border rounded-md px-2 py-1 text-[12px] font-semibold text-ink outline-none cursor-pointer focus:border-iris-500"
                            >
                                {Array.from(
                                    { length: 16 },
                                    (_, i) => i + 1,
                        ...
                        <div className="max-h-[280px] overflow-y-auto border border-border rounded-lg text-[12px]">
                            <table className="w-full text-left">
                                <thead className="bg-surface2/80 sticky top-0 font-bold border-b border-border">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-border/60">
                                    {enrolledStudents.map((std) => (
                                        <tr
                                            key={std.id}
                                            className="hover:bg-surface2/25"
                                        >
                                            <td className="p-2.5">
                                                <span className="font-bold text-ink block">
                                                    {std.name}
